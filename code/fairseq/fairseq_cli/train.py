...
def validate_and_save(args, trainer, task, epoch_itr, valid_subsets):
    ...
    valid_losses = [None]
    if do_validate:
        valid_losses, best_valid = validate(args, trainer, task, epoch_itr, valid_subsets)
        # if best, save
        do_save = (do_save or best_valid)
...
def validate(args, trainer, task, epoch_itr, subsets):
    ...
    for subset in subsets:
        ...
        # added by Michael Xie
        # trigger should_save for any subset that is best
        best_key = f"best_{args.best_checkpoint_metric}"
        if best_key not in stats or (stats[args.best_checkpoint_metric] == stats[f"best_{args.best_checkpoint_metric}"]):
            should_save = True

        valid_losses.append(stats[args.best_checkpoint_metric])
    # return valid_losses
    return valid_losses, should_save
