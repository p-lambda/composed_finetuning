#########
# HELPER CODE taken from SPoC paper
#########
...
# Global arguments
# ARGS = None
class DefArgs(NamedTuple):
    gcc_timeout = 60
    timeout = 2
    prog_dir = str((Path(__file__).parent / 'data/spoc-data/testcases').resolve().expanduser())
ARGS = DefArgs()
...
def compare_files(outfile, prgfile):
    ...
    with open(prgfile, 'br') as fin:
        prg_outdata = fin.read()
        # print(f"PRED: {prg_outdata} | GOLD: {outdata}")
        return prg_outdata == outdata
...
def stitch_prefix_pruning(inp_stmt, pred_stmt, probid, subid):
    ...
    while not heap.empty() and prefix_ccount + heap_ccount < ARGS.compile_budget:
        ...
        # check_flag, check_value = check_if_bad(curr_idx, bad_prefixes)
        # if check_flag:
        #     skip_ccount += 1
        #     stat_file.write("BAD PREFIX : " + str(check_value) + ", skipping..." + "\n")
        #     stat_file.write("continuing best first search...\n\n")
        #     continue
        ...
        for inp, pred in zip(inp_stmt, pred_stmt):
            ...
            if pred[_pred.text] == 'DUMMY':
                ...
                idx_count += 1
        # run the program
        # stat_file.write("########### HEAP PROPOSED CODE #############\n")
        # stat_file.write(code + "\n")
        # stat_file.write("########### HEAP PROPOSED CODE ENDS #############\n")
        ...
        if error != err.compile_err:
            ...
            if err_line is None:
                ...
                if err_line != len(prob_list) - 1:
                    ...
                    # stat_file.write("########### CURRENT PREFIX SEARCH #############\n")
                    # stat_file.write(code + "\n")
                    # stat_file.write("########### CURRENT PREFIX SEARCH ENDS #############\n")
        ...
        for i in range(add_to_heap_upto):
            ...
            # l_idx = 0
            # r_idx = len(curr_idx) - 1
            # bin_search_iter = 1
            # while l_idx <= r_idx:
            #     stat_file.write("Binary search iteration #" + str(bin_search_iter) + "\n")
            #     stat_file.write("l_idx: " + str(l_idx) + "\n")
            #     stat_file.write("r_idx: " + str(r_idx) + "\n")
            #     mid_idx = l_idx + (r_idx - l_idx) // 2
            #     stat_file.write("mid_idx: " + str(mid_idx) + "\n")
            ...
            #     # stat_file.write("########### CURRENT BINARY SEARCH #############\n")
            #     # stat_file.write(code + "\n")
            #     # stat_file.write("########### CURRENT BINARY SEARCH ENDS #############\n")
            ...
            # stat_file.write("Binary search exited" + "\n")
            # stat_file.write("l_idx: " + str(l_idx) + "\n")
            # stat_file.write("r_idx: " + str(r_idx) + "\n")

            # stat_file.write("BAD PREFIX: " + str(curr_idx[:l_idx + 1]) + "\n")
            # if (l_idx + 1) not in bad_prefixes:
            #     bad_prefixes[(l_idx + 1)] = set()
            # bad_prefixes[(l_idx + 1)].add(tuple(curr_idx[:l_idx + 1]))
            # stat_file.write("added to set..." + "\n")
...
def main():
    ...
    parser.add_argument('--prog-dir', default='/u/scr/nlp/synlp/synlp/codeforces-data',
    ...
    if os.environ.get('PROG_DIR'):
        ARGS.prog_dir = os.environ['PROG_DIR']
