...
def pad_before(tokens, pad_idx, eos_idx, return_orig_mask=False, add_eos=True):
    ...
    for tok_list in tokens:
        if add_eos:
            ...
            new_tokens.append(torch.cat([(pad_idx * torch.ones(max_len - len(tok_list))).long().cuda(), tok_list]))

        if return_orig_mask:
            if add_eos:
                new_mask.append(
                        torch.cat(
                            [torch.zeros(max_len - len(tok_list)).bool().cuda(),
                             torch.ones(len(tok_list)).bool().cuda(),
                             torch.tensor([0]).bool().cuda()
                             ]))
                orig_mask.append(
                        torch.cat(
                            [torch.ones(len(tok_list)).bool().cuda(),
                ...
                orig_mask.append(
                        torch.cat(
                            [torch.ones(len(tok_list)).bool().cuda(),
                             torch.zeros(max_len - len(tok_list)).bool().cuda(), ]))
    if return_orig_mask:
        return torch.stack(new_tokens), torch.stack(orig_mask), torch.stack(new_mask)
    else:
...
@register_criterion('straightthrough_criterion')
class StraightThroughCriterion(FairseqCriterion):
    '''
    Straight through estimation of the composed fine-tuning gradient instead
    of REINFORCE. We found similar performance in synthetic code, but did
    not run for SPoC.
    '''
    ...
    def get_stacked_loss(self, model, sample, target, reduce=True):
        # this function implicitly makes the model go to eval mode
        _, _, y_encout_log = model._get_sample_simple(**sample['net_input'], training=True)
        model.train()
        y_encout = F.softmax(y_encout_log, dim=2)
        y_encout_flat = y_encout.view(-1, y_encout.shape[2])

        # argmax
        yhat = y_encout.argmax(2)
        # fix padding
        yhat_pad, orig_mask, new_mask = pad_before(yhat, self.padding_idx, model.generator.eos, return_orig_mask=True, add_eos=False)
        yhat_lens = get_lens(yhat_pad, self.padding_idx)

        # flatten
        yhat_pad_flat = yhat_pad.view(-1)
        yhat_pad_onehot = torch.zeros(yhat_pad_flat.shape[0], y_encout.shape[2]).cuda()
        yhat_pad_onehot.scatter_(1, yhat_pad_flat.unsqueeze(1), 1)
        # straight through to y_encout
        orig_mask = orig_mask.view(-1)
        new_mask = new_mask.view(-1)
        yhat_pad_onehot[new_mask] = y_encout_flat[orig_mask] + (yhat_pad_onehot[new_mask] - y_encout_flat[orig_mask]).detach()

        # embed
        embedding = model.encoder_2.embed_tokens
        yhat_embedding = torch.matmul(yhat_pad_onehot, embedding.weight).view(yhat_pad.shape[0], yhat_pad.shape[1], -1)

        net_output = model(yhat_pad, yhat_lens, sample['net_input']['prev_output_tokens'], output_mode='second', token_embedding=yhat_embedding)
        ...
        stacked_pred = lprobs.max(1)[1]
        stacked_acc = (stacked_pred == target.view(-1)).sum().float()
        return loss, nll_loss, stacked_acc
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss, nll_loss, stacked_acc = self.get_stacked_loss(model, sample, target, reduce=reduce)

        if self.ce_loss_lambda > 0.0:
